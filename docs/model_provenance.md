...
| Team style metrics | `data/source_cache/squad_style/latest_metrics.json` → `team_metrics` | Filled per matchday by the deterministic ESPN collector (`collect_espn_matchday.py`): possession, shots/90, shots-on-target %, passes/90, pass completion %, goals/90, goals conceded/90, shots against/90. ESPN does not expose xG, so xG/PPDA/field-tilt stay missing. Squad market value/age remain Transfermarkt-sourced. | `web_researched` (ESPN match boxscore) or `missing` |
| Radar (Team Performance Metrics) | `data/quality.radar_metrics` over `team_metrics` | Uses the four metrics the ESPN feed provides for every played match: possession, shots/90, shot accuracy (SoT %), pass completion %. Renders only when all four are present for both teams. | `web_researched` or `unavailable` |
| Matchday lineups | `data/source_cache/lineups/latest.json`, merged into `/api/match/{id}/summary` (T-046) | Confirmed starting XIs + formations from ESPN match data (collector), surfaced as slug-keyed `rosters` + `player_clubs`. Refreshed each matchday by the automated collector. | `web_researched` |
