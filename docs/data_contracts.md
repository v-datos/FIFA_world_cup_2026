...
| `data/source_cache/squad_style/latest_metrics.json` | Active source cache | T-038 field-level squad/style metric source cache. The API consumes cached fields only; unsupported fields remain hardcoded reference or missing with field-level status metadata. |
