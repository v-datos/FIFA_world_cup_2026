...
Task: T-024 - Data Contract Audit for Active JSON and API Payloads; T-026/T-028/T-032/T-033/T-034/T-036/T-038/T-039/T-040 provenance addenda
...
- Implemented briefing API/freshness UI contract from T-033.
...
| Runtime API augmentation | Active derived payload | `/api/match/{id}/metrics` adds `elo_ratings`, `monte_carlo_projections`, `viz_proxies`, and `data_quality` at request time. `/api/match/{id}/summary` adds `briefing_status` at request time. `/api/match/{id}/briefing` returns the full briefing payload or a safe baseline/invalid fallback. |
...
| `data/source_cache/squad_style/latest_metrics.json` | Active source cache | Field-level squad/style metric source cache (T-038). Refreshed each matchday by the deterministic ESPN collector `src/pipeline/collect_espn_matchday.py` (possession, shots, shots-on-target, passing, goals). The radar requires `possession_avg`, `shots_per_90`, `shots_on_target_pct`, `pass_completion_pct`; xG/PPDA/field-tilt are not provided by ESPN and stay missing. |
| `data/source_cache/lineups/latest.json` | Active source cache | Matchday starting XIs + formations, refreshed by the ESPN collector; merged into `/api/match/{id}/summary` as `rosters` + `player_clubs`. |
...
## `latest_metrics.json` Squad/Style Source Cache Contract

Path: `data/source_cache/squad_style/latest_metrics.json`
Dry-run inspector: `src/pipeline/collect_squad_style_sources.py`
Runtime reader: `src.analytics.squad_style_sources`

Purpose:

- Adds field-level source-backed squad/style metric values without rewriting
  curated `data/matches/{match_id}/metrics.json` files.
- Allows `/api/match/{id}/metrics.data_quality.team_metrics[team].fields`
  to distinguish source-backed, hardcoded reference, and missing fields.
...
| Field | Type | Notes |
|---|---|---|
| `metadata` | object | Source, parser, coverage, warning, and blocked-reason metadata. |
| `teams` | array | Team source records scoped by optional `fixture_ids`. |
...
| `warnings` | array | Human-readable limitations. |
| `blocked_reasons` | array | Machine-readable collection blocks. |
...
| Field | Type | Notes |
|---|---|---|
| `value` | number | Runtime value merged into the API payload for this field only. |
| `unit` | string | Example: `EUR_m` or `years`. |
| `status` | string | Current value: `available`. |
| `source_label` | string | Usually `web_researched`; use `proxy_historical` only with explicit approximation metadata. |
| `source_name` | string | Source display name. |
| `source_url` | string | Auditable source URL. |
| `source_path` | string | Human-readable source location. |
| `source_value_text` | string | Short source text/value used to derive the numeric value. |
| `checked_at_utc` | string | UTC source-check timestamp. |
| `retrieval_method` | string | Current value: `manual_static_manifest`. |

Operational rules:

- The API reads this cache only from disk; it does not fetch source pages at
  request time.
- Cached fields may override the runtime value for that same field, but the
  checked-in `metrics.json` file is not rewritten.
- Fields without cache records are not inferred. They remain `hardcoded_reference`
  when stored values exist and `missing` when no stored value exists.
- Any future proxy-derived field must set `approximation=true`,
  `approximation_note`, and `source_label=proxy_historical`.
...
Current API exposure: `/api/match/{match_id}/briefing` returns the full
artifact or a safe fallback payload. `/api/match/{match_id}/summary` adds the
same compact `briefing_status` for compatibility.
...
- Missing `briefing.json` returns HTTP 200 with `freshness_state=baseline_only`,
  `artifact_status=missing`, and a static `summary.json` source record.
- Invalid or unreadable `briefing.json` returns HTTP 200 with
  `freshness_state=invalid`, `artifact_status=invalid`, and blocked/invalid
  warning metadata.
- `fresh` artifacts whose `metadata.valid_until_utc` has passed are downgraded
  to `stale` at response time.
- The dedicated route validates required top-level blocks before exposing an
  artifact as available.
- The route does not mutate `summary.json`, `metrics.json`, or `briefing.json`.
...
| `team_metric_source_cache` | object | T-038 source-cache metadata from `data/source_cache/squad_style/latest_metrics.json` | API consumers that need cache status and coverage details. |
| `team_metric_sources` | object keyed by team, then field | T-038 source records applied at runtime | API consumers that need field-level source metadata separate from quality summaries. |
...
states; T-038 added the field-level source cache and first partial sample;
T-031 remains for broad value completion. T-026 completed the model/provenance
review and routed source-policy decisions to T-035.
...
Runtime labels are now added by T-028, T-032/T-033, T-038, and T-039.
Remaining source-backed metric fields can be added incrementally by T-031 and
future source-collection tasks. Existing JSON remains valid until those tasks
update stored schemas and consumers.
...
| Field | Meaning |
|---|---|
| `freshness_state` | `fresh`, `stale`, `baseline_only`, `blocked`, `skipped`, or `invalid`. Missing `briefing.json` renders as `baseline_only`. |
| `source_label` | Primary provenance label such as `static_curated`, `web_researched`, `live_schedule`, or `blocked`. |
| `source_labels` | Ordered distinct source labels found on the artifact, when available. |
| `generated_at_utc` | Briefing generation timestamp, or null for baseline-only fallback. |
| `valid_until_utc` | Last-minute validity window end, or null for baseline-only fallback. |
| `checked_at_utc` | First available source check timestamp, artifact generation time, or null. |
| `review_status` | Review gate status from the artifact, `not_generated`, or `invalid`. |
| `artifact_status` | `available`, `missing`, or `invalid`. |
| `artifact_path` | Project-relative target path for the optional `briefing.json`. |
...
- `briefing_status` is derived through the same helper used by the dedicated
  `/api/match/{match_id}/briefing` route.
- When `briefing.json` exists, the route validates the required top-level
  blocks and reads freshness from `data_quality.freshness_state`,
  `metadata.freshness`, or the legacy `metadata.freshness_state` fallback.
- Source label is derived from `sources[].label` or `sources[].source_label` and
  uses `blocked` for blocked/skipped briefing states.
- Missing `briefing.json` remains `baseline_only`.

### `GET /api/match/{match_id}/briefing`

Returns a full `briefing.json` payload when a valid artifact exists. If no
artifact exists, returns a contract-compatible fallback payload that preserves
the static baseline preview context.

Top-level response blocks:

| Block | Meaning |
|---|---|
| `metadata` | Artifact metadata or baseline fallback metadata. |
| `fixture` | Fixture metadata copied from `summary.json`. |
| `team_keys` | Canonical team slugs for both sides. |
| `briefing` | Draft briefing content, or null for baseline-only fallback. |
| `forecast_snapshot` | Forecast snapshot from the generated artifact, or empty fallback. |
| `data_quality` | Freshness, warnings, and blocked reasons. |
| `sources` | Source records used by the artifact or the static `summary.json` fallback. |
| `review` | Review gate state. |
| `briefing_status` | Compact status object used by the frontend badge. |

Failure and fallback behavior:

- Returns 404 with `Summary payload not found` if the fixture has no
  `summary.json`.
- Returns HTTP 200 with `freshness_state=baseline_only` and
  `artifact_status=missing` when `briefing.json` does not exist.
- Returns HTTP 200 with `freshness_state=invalid` and
  `artifact_status=invalid` when `briefing.json` cannot be read or fails the
  required-block validation.
- Downgrades expired `fresh` artifacts to `stale` at response time based on
  `metadata.valid_until_utc`.

Frontend consumption:

- `MatchAnalysisTab` fetches this route alongside summary and metrics.
- If the dedicated route is unreachable, the frontend falls back to
  `summary.briefing_status`.
- The `BriefingFreshnessBadge` renders fresh/stale/baseline/blocked/skipped/
  missing/invalid states without hiding the static tactical preview.
...
| Squad & Style Comparison | 15 `team_metrics` fields plus `metrics.elo_ratings[team]` and `metrics.data_quality.team_metrics`; missing/partial states are visible. Runtime Elo can now be `web_researched` through the T-039 cache, and individual squad/style fields can be `web_researched` through the T-038 cache. |
